"""TODOS:
* Load list from file.
* Time out raw_input.
* Make the print statements more decorative.
* Optimise the string checking algo.
* Add instructions to play.
* Log and display the time taken for individual word and the time spent on the game.
* Better expcetion Handling."""

from random import choice
from timeit import default_timer as timer
import json


class WordGuess(object):

    def __init__(self, word_list, user="anonymous"):
        self.user = user
        self.word_list = word_list
        self.won = 0
        self.lost = 0
        self.score = 0
        self.word_time_dict = {}

    def play_game(self):
        pick_word = choice(self.word_list)
        guess_word = list(pick_word)
        guess_word_len = len(guess_word)
        turns_allowed = guess_word_len + 3

        answer_list = ["__ "] * guess_word_len
        print(answer_list)

        start_time = timer()
        while(turns_allowed):
            user_guess_letter = raw_input("Enter a char:: ")
            if user_guess_letter is None and not user_guess_letter.isalpha():
                turns_allowed = turns_allowed - 1
                continue
            for index, char in enumerate(guess_word):
                if char == user_guess_letter:
                    answer_list[index] = char
                    self.score += 0.5
            print(answer_list)
            if answer_list == guess_word:
                end_time = timer()
                print("## You Won! ##")
                self.score += 2 + turns_allowed
                self.won += 1
                break
            turns_allowed = turns_allowed - 1
        else:
            end_time = timer()
            print("@@ You Lost! @@")
            self.lost += 1
            self.score -= 0.3
        self.word_list.remove(pick_word)
        self.word_time_dict[pick_word] = end_time - start_time

    def play_again(self):
        self.play_game()
        play_more = raw_input("Enter y to play:: ")
        while(play_more.lower()[0:1] == "y" and len(self.word_list) > 0):
            self.play_game()
            play_more = raw_input("Enter y to play:: ")
        if len(self.word_list) == 0:
            print("END OF GAME: All words exhausted")

        print("Hey %s , well played!" % self.user)
        print("You played %s games" % str(self.won + self.lost))
        print("You won %s games" % str(self.won))
        print("You lost %s games" % str(self.lost))
        print("Your score is %f" % self.score)


if __name__ == '__main__':
    user = "github"
    word_list = ['sarah', 'masud', 'google']
    game = WordGuess(word_list, user)
    game.play_again()
